...
template <class T>
const size_t Component<T>::COMPONENT_TYPE_ID =
    TypeCounter<IComponent>::Get<T>();
...
class ComponentContainer : public IComponentContainer {
 ...
 public:
  ComponentContainer() {
    for (int i = 0; i < MAX_COMPONENTS; i++) freeBlocks.push_back(i);
  }
...
ComponentTypeBitset getComponentTypeBitset(
    std::initializer_list<ComponentTypeId> ctIds) {
